...
def parameter_to_simulation(i, parameter_list_full, year_max):
    ...
    for column_index, column_name in enumerate(parameter_list_full.columns):
        parameter_dict[column_name[0]
                       ] = parameter_list_full[column_name[0]].iloc[i].item()
...
if __name__ == '__main__':
    ...
    parameter_history = pd.DataFrame(columns=[
                                     "changed parameter", "previous value", "next value", "NRMSD_min", "location", "relative change"])
    ...
    print('Estimated time for one zoom = ' +
          str(round(len(parameter_list_full)*0.51)) + ' seconds')
    print('Estimated time till end conditions are met = ' +
          str(round(len(parameter_list_full)*0.51 * s.analysis_number_end_condition)) + ' seconds')
    ...
    while stop_condition == True:
        ...
        print('Number of simulations completed = ' +
              str(len(parameter_list_full) * (analysis_number-1)))

        # run simulation
        df_results = pd.DataFrame()
        results = [pool.apply_async(parameter_to_simulation, args=(
            i, parameter_list_full, s.year_max)) for i in range(0, no_of_simulations)]
        ...
        for i in range(0, no_of_simulations):
            metric_result = af.calculate_metrics_multiple_attributes(
                df_results, empirical_data, str(i+1))
        ...
        print(str(s.variable_to_improve) + " min = " +
              str(round(metrics[s.variable_to_improve].min(), 8)))

        # function which calculates the next limits of improved parameter and saves certain values
        parameter_list, parameter_history_temp = af.improved_limits(
            metrics, parameter_list, parameter_list_full)

        # create parameter list full with nrmsd values
        nrmsd_total = pd.DataFrame(data=metrics["NRMSD_total"])
        parameter_list_full_nrmsd = parameter_list_full

        parameter_list_full_nrmsd = pd.concat([parameter_list_full_nrmsd.reset_index(drop=True),
                                               nrmsd_total.reset_index(drop=True)], axis=1)

        # append parameter_history_temp to parameter_history
        parameter_history = pd.concat(
            [parameter_history, parameter_history_temp], ignore_index=True)

        print("Improved parameter = " +
              parameter_history.iloc[analysis_number-1, 0])
        print("From value: " + str(parameter_history.iloc[analysis_number-1, 1]) + " to value: " + str(
            parameter_history.iloc[analysis_number-1, 2]))
        ...
        change = round(abs(parameter_history.iloc[analysis_number-1, 1] -
                       parameter_history.iloc[analysis_number-1, 2])/parameter_history.iloc[analysis_number-1, 1], 6)
        ...
        if analysis_number > 1:
            delta_nrmsd = abs(
                parameter_history.iloc[analysis_number-1, 3]-parameter_history.iloc[analysis_number-2, 3])
        ...
        if delta_nrmsd < s.nrmsd_delta_end_condition or analysis_number >= s.analysis_number_end_condition:
            if delta_nrmsd < s.nrmsd_delta_end_condition:
                print("\n\nEnd condition NRMSD delta")
            if analysis_number >= s.analysis_number_end_condition:
                print("End condition analysis number")
    ...
    # print minimal nrmsd
    print("NRMSD min: " +
          str(parameter_history.iloc[len(parameter_history)-1, 3]))
    ...
    parameter_improved_value_list.to_excel(
        "Analyse Ergebnisse/Analysis parameter_list_{}.xlsx".format(date_time))
    # save parameter_history results in excel list
    parameter_history.to_excel(
        "Analyse Ergebnisse/Analysis parameter_history_{}.xlsx".format(date_time))
    # save settings
    settings_list = af.get_settings_list()
    settings_list.to_excel(
        "Analyse Ergebnisse/Settings_list_{}.xlsx".format(date_time))
